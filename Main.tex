...
% 标题
\title{LODS-MTI: A Link-Adaptive, Orthogonal, and De-slotted Protocol for Robust and Fast RFID Missing Tag Identification}
...
\email{xxxxx@163.com}                    % 通讯邮箱
\firstauthor{Hongquan Zhou}                   % 第一作者
\femail{xxxxxxxx@qq.com}                    % 第一作者邮箱
...
Welcome to the \textbf{Academic Paper Template}. This template is designed for academic preprints and articles, featuring a clean, professional design. It is built on the `extarticle` class and fully supports XeLaTeX compilation.
...
\begin{enumerate}
    ...
    \item \textbf{Class Files}: The core logic resides in the `class/` directory (`main.cls`, `sup.cls`, `cover\_letter.cls`, `response.cls`). Do not modify these unless you need to change the global style.
...
\begin{itemize}
    \item \textbf{Header}: Displays the Running Title (`\textbackslash title`) and Lead Author (`\textbackslash leadauthor`).
    \item \textbf{Footer}: Contains the Institution Name (`\textbackslash institution`), Version Date (automatically generated), and Page Number (e.g., ``1--5'').
    \item \textbf{Metadata}: The bottom rule contains the Correspondence (`\textbackslash corres`), License/Funding (`\textbackslash license`), and Dates (`\textbackslash dates`). Ensure `\textbackslash setbool\{corres-info\}\{true\}` is set to enable this block.
\end{itemize}
...
\section{Conclusion}
This template aims to provide an ``out-of-the-box'' experience for professional academic writing. If you encounter missing footer information, please check that `\textbackslash setbool\{corres-info\}\{true\}` is present in your preamble.
